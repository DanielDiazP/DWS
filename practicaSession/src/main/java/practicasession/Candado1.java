...
@WebServlet(name = "Candado1", urlPatterns = {"/candado1"})
public class Candado1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String nivelSalida="0";

        if (request.getSession().getAttribute(Constantes.ATTRIBUTE_NAME) == null) {
            request.getSession().setAttribute(Constantes.ATTRIBUTE_NAME, nivelBase);
        }

        if (request.getSession().getAttribute(Constantes.ATTRIBUTE_NAME).equals(nivelBase)) {
